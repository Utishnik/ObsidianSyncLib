...
fn test_skip_construction() {
    ...
    let result1: bool = obsidian_sync_lib::tokinezed::skip_construction(
        "      { { ",
        &mut 0,
        " ",
        "{{",
    ...
    println!(
        "\nSTART:  {}\tEND:  {}\n",
        construct.start.unwrap_or(defult_val),
        construct.end.unwrap_or(defult_val)
    );
    ...
    construct.monolit = false;
    let result1: bool =
        obsidian_sync_lib::tokinezed::skip_construction("{{", &mut 0, " ", "{{", &mut construct);
    println!("{}", result1);
    println!(
        "\nSTART:  {}\tEND:  {}\n",
        construct.start.unwrap_or(defult_val),

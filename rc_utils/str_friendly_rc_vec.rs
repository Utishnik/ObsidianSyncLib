//https://github.com/murarth/ketos/blob/master/src/ketos/rc_vec.rs взят отсюда
...
impl RcString {
    ...
    pub fn slice<R: RangeArgument<usize>>(&self, range: R) -> RcString {
        let start: usize = range.start().map_or(0, |v| *v);
        let end: usize = range.end().map_or(self.len(), |v| *v);

        let a: usize = self.start + start;
        let b: usize = self.start + end;
    ...
    fn make_mut(&mut self) -> &mut String {
        let s: &mut String = Rc::make_mut(&mut self.data);
...
impl<T: Clone> RcVec<T> {
    ...
    fn make_mut(&mut self) -> &mut Vec<T> {
        let v: &mut Vec<T> = Rc::make_mut(&mut self.data);
...
mod test {
    ...
    #[test]
    fn test_rc_string() {
        let a: RcString = RcString::from("foobar");
        let mut b: RcString = a.slice(1..4);
        let mut c: RcString = a.clone();

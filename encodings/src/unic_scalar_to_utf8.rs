//https://github.com/rust-lang/regex/blob/master/regex-syntax/src/utf8.rs
/*!
...
pub struct Utf8RangeVec<'a>(Vec<&'a Utf8Range>);
pub struct Utf8RangeVecOwned(Vec<Utf8Range>);
//TODO для ArrayVec
...
impl fmt::Debug for Utf8RangeVecOwned {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        for item in self.0.iter() {
            let format: alloc::string::String = format!("{item:?}");
            writeln!(f, "{format}")?
...
impl ScalarRange {
    ...
    fn as_ascii(&self) -> Option<Utf8Range> {
        if self.is_ascii() {
            let start: u8 = u8::try_from(self.start).unwrap();
            let end: u8 = u8::try_from(self.end).unwrap();
    ...
    fn encode(&self, start: &mut [u8], end: &mut [u8]) -> usize {
        let cs: char = char::from_u32(self.start).unwrap();
        let ce: char = char::from_u32(self.end).unwrap();
        let ss: &mut str = cs.encode_utf8(start);
        let se: &mut str = ce.encode_utf8(end);

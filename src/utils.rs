...
//todo через where
pub fn unique_sym_to_vec<T: std::ops::Add + std::clone::Clone + std::cmp::PartialEq + std::default::Default>(sl1: &[T],sl2: &[T]) -> Vec<T>
{
    ...
    if max_slice(sl1, sl2) < 2*min_slice(sl1, sl2){ //типо чтоб не создавать большой каписити если разница огромная
        result.reserve(sum_slice_len(sl1,sl2));
    }
    else{
        debug_println_fileinfo!("sl1 len = {}\tsl2 len = {}\n[может стоить Распараллелить?)]",sl1.len(),sl2.len());
    }

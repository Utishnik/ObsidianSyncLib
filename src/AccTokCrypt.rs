...
pub fn decrypt_acctok(crypt_acctok: &[u8],security_param: SecurityParam) -> Result<String,Box<dyn Error>>
{
    let res: Result<(), Box<dyn Error>> = load_env_file(security_param.clone());
    match res 
    {
        Ok(_) => {
          let acctok:String = String::from_utf8_lossy(&xor_decrypt(crypt_acctok,security_param.password.as_bytes())).to_string();
          Ok(acctok)  
        },
        Err(error) => Err(error),
    }
...
pub fn crypt_acctok(decrypt_acctok: &[u8],security_param: SecurityParam) -> Result<String,Box<dyn Error>>
{
    let res: Result<(), Box<dyn Error>> = load_env_file(security_param.clone());
    match res 
    {
        Ok(_) => {
          let encrypt_acctok:String = String::from_utf8_lossy(&xor_encrypt(decrypt_acctok,security_param.password.as_bytes())).to_string();
          Ok(encrypt_acctok)  
        },
        Err(error) => Err(error),
    }
...
pub fn create_env_dir(obsidian_rep_name: String,sec_par: &mut SecurityParam) -> Result<(),std::io::Error>
{
    ...
    else { //ебаный раст не может понять что оба случая обработаны и это условие не нужно
        Ok(())
    }
}

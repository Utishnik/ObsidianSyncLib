...
impl Config {
    ...
    pub fn set_value(&self, value: &str) -> Result<(), String> {
        let mut guard = self.config.write().map_err(
            |e: std::sync::PoisonError<std::sync::RwLockWriteGuard<'_, String>>| {
    ...
    pub fn get_value(&self) -> Result<String, String> {
        if !self.init_check().is_empty() {
            ...
            let guard: std::sync::RwLockReadGuard<'_, String> = self.config.read().map_err(
                |e: std::sync::PoisonError<std::sync::RwLockReadGuard<'_, String>>| {

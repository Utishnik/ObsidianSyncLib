...
impl Pos {
    ...
    pub fn conver_to_idx(&self,cfg: &str) -> usize{
        let result: usize=0;
        splitt_b_space(cfg.clone().to_string(), None, Some("\n".to_string()));
        result
    }
}
...
pub fn smart_abstract_multiconstruction_value_parse<F, T,E>(fns: Vec<F>,idx: &mut usize) -> Vec<AbstractParseValue<T,E>>
where
...
pub fn get_abstract_value_in_vec<T,E>(vec: Vec<AbstractParseValue<T,E>>,idx: usize) -> Option<AbstractParseValue<T,E>>
where 
T: AbstractValue + std::clone::Clone,
E: std::clone::Clone,
{

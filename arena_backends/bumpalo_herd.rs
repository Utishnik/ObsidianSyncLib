...
impl Herd {
    ...
    pub fn get(&self) -> Member<'_> {
        let mut lock: std::sync::MutexGuard<'_, Vec<Box<Bump>>> = self.0.lock().unwrap();
        let bump: Box<Bump> = lock.pop().unwrap_or_default();
        Member {
...
impl Drop for Member<'_> {
    fn drop(&mut self) {
        ...
        let mut lock: std::sync::MutexGuard<'_, Vec<Box<Bump>>> = self.owner.0.lock().unwrap();
        /*
         ...
         */
        let member: Box<Bump> = unsafe { ManuallyDrop::take(&mut self.arena) };
        lock.push(member);
    }
}
